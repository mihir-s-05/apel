...
\begin{tikzpicture}[
  ...
  smallbox/.style={draw, rounded corners, align=center, minimum width=28mm, minimum height=7mm},
  arrow/.style={-Latex, thick},
]
  \node[box] (tokens) {Tokens\\$y_{\le t}$};
  \node[box, right=of tokens] (gru) {Executor\\GRU backbone\\$h_t$};
  \node[box, right=of gru] (experts) {Expert heads\\$p_k(\cdot\mid h_t)$};

  \node[smallbox, below=of gru, xshift=10mm] (belief) {Planner belief\\$\B_t\in\Delta^{K-1}$};
  ...
  \node[box, right=of experts, xshift=5mm] (mix) {Mixture\\$p(y_{t+1})=\sum_k g_t(k)p_k$};
