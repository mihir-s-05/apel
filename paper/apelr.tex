...
Finally, we outline a practical scaling roadmap from the current small/medium prototype regime to $\sim$1B-parameter training.
\end{abstract}
...
\begin{tikzpicture}[
  ...
  % Place this label to the left to avoid pushing the figure bounding box past the page margin.
  \draw[arrow] (sample) |- node[pos=0.25, left, align=right, font=\small] {token filtering\\update $\B_{t+1}$} (belief);
...
\caption{Schematic of \APELR{} (mainline implementation). The executor produces hidden states; experts produce state-conditional token distributions; the planner maintains an online belief over discrete plan states; optional lookahead forecasting can be computed asynchronously and fed back into the gating distribution.}
...
\begin{enumerate}
  ...
  \item We outline a practical engineering and experimental roadmap for scaling \APELR{} beyond the current prototype regime toward $\sim$1B parameters (\Cref{sec:scaling}).
\end{enumerate}
\end{samepage}
...
\begin{proposition}[Stale chunk mixture is generally approximate]
\label{prop:stale-not-exact}
Fix a token position $t\in\mathcal{I}_c$ and define state-conditional emission vectors
$p_z(\cdot):=p_\theta(\cdot\mid x,y_{<t},z_c=z)$.
Let $p(\cdot)=\sum_z \bvec_{c,t}(z)p_z(\cdot)$ denote the exact conditional, and let $\tilde p(\cdot)=\sum_z \bvec_c(z)p_z(\cdot)$ denote the stale-belief mixture used in Mode C.
If $\{p_z(\cdot)\}_z$ are linearly independent in $\R^{|\V|}$ (a standard mixture identifiability condition), then $p=\tilde p$ implies $\bvec_{c,t}=\bvec_c$.
Without identifiability, equality can hold only in degenerate cases where different weights induce the same mixture (e.g., identical emissions).
\end{proposition}
\begin{proof}[Sketch]
By definition,
$p(\cdot)-\tilde p(\cdot)=\sum_z(\bvec_{c,t}(z)-\bvec_c(z))\,p_z(\cdot)$.
If the emission vectors are linearly independent, the only coefficient vector yielding the zero vector is $\bvec_{c,t}-\bvec_c=0$.
\end{proof}
...
\section{\APELR: Discrete Planner with Expert-Only Emission}
\label{sec:apelr-arch}
We now describe the mainline repository implementation, emphasizing what is mathematically exact versus a deliberate engineering choice.
...
The repository contains a legacy filtered-mixture variant for historical comparison; throughout this paper, \APELR{} refers to the expert-only emission implementation in \texttt{src/apelr/model\_v2.py}.

\subsection{Planner state: discrete latent belief}
In \APELR{}, the planner state is a discrete distribution over \emph{latent} states (not literal tokens).
...
\]
\paragraph{Expert-only emission.}
In \APELR{} (\texttt{src/apelr/model\_v2.py}), each planner state has its own linear head,
$a_{t,k}=W_k h_t + b_k$.
This ``expert-only'' design removes a high-capacity shared bypass path and empirically increases planner intervention sensitivity (forced-state divergences) at comparable training budgets, while retaining a clean mixture-of-experts interpretation \citep{shazeer2017moe}.
...
\footnote{If $\tau=1$ and $\lambda=0$ so that the emission gate equals the belief ($g_t=\B_t$), then \Cref{eq:apelr-token-update} coincides with the exact Bayesian filtering update for a discrete latent mixture. For $\tau\ne 1$ and/or $\lambda>0$, the update remains a valid causal state update but should not be interpreted as the exact posterior of the emitted mixture weights without an augmented generative story.}
...
Importantly, training and generation both apply the same boundary update semantics.
...
\APELR{} avoids this by defining the boundary update strictly as a function of the \emph{previous} completed chunk summary.
...
\APELR{} defines a mixed belief used for gating:
\[
...
In \APELR{} generation (\texttt{generate\_planned}), lookahead computation can be offloaded to a background CPU thread:
...
\begin{algorithm}[t]
\caption{\APELR{} generation with optional async lookahead}
\label{alg:gen}
\begin{algorithmic}[1]
...
In \APELR{}, the loss computation follows the same online recursion as generation (including prompt ingestion semantics and causal boundary updates), addressing a common failure mode where training uses information unavailable at inference time.
...
\APELR{} supports two training-time regimes for the chunk-level planner state representation:
\begin{itemize}
...
We regularize the entropy of chunk-start posteriors
$H(\B)= -\sum_k \B(k)\log(\B(k){+}\varepsilon)$.
In the current implementation, this term is added to the loss with a positive weight, which encourages \emph{lower} entropy (sharper chunk beliefs) and thus stronger expert commitment; flipping the sign encourages more diffuse beliefs early in training.
...
The objective used in this repository is:
\begin{equation}
\label{eq:v2-total}
\mathcal{L}
= \mathcal{L}_{\mathrm{nll}}
...
For \APELR{}, evaluation also computes planner diagnostics (mask deltas, forced-state divergences, utilization) and the lookahead feedback delta (\Cref{sec:v2-losses}).
...
\subsection{Hardware}
All CUDA runs reported here were executed on a single laptop GPU:
NVIDIA GeForce RTX 4070 Laptop GPU (8\,GB VRAM), using PyTorch 2.6.0 (\texttt{+cu124}) with CUDA runtime 12.4 and fp16 autocast.

\subsection{Compute and FLOPs (Transformer-style reporting)}
\label{sec:compute}
We report rough forward-pass floating point operation (FLOP) counts and memory probes in the style of standard Transformer papers \citep{vaswani2017attention}.
Unless noted, FLOPs count multiply--add as 2 FLOPs and ignore layernorm/nonlinearities.

\paragraph{\APELR{} (GRU + $K$ expert heads).}
For a GRU layer with input size $I$ and hidden size $H$, the dominant per-token cost is approximately
$6H(I{+}H)$ FLOPs (3 gates, input-to-hidden and hidden-to-hidden).
With $L$ GRU layers, the backbone cost is $\sum_{\ell=1}^{L}6H(I_\ell{+}H)$.
Expert emission uses $K$ linear heads $W_k\in\R^{|\V|\times H}$, costing approximately $2K|\V|H$ FLOPs per token.
Planner belief updates and $K\times K$ lookahead are negligible at the reported $K=8$.

\paragraph{Transformer baseline.}
For a decoder-only Transformer with model width $d$, sequence length $T$, and $L$ layers, a rough per-token forward cost is:
per layer, $24d^2 + 4Td$ FLOPs (QKV/out projections + MLP + attention matmuls), plus a final LM head cost $2|\V|d$.

\paragraph{Numbers for the WikiText-2 configs (seq\_len 96).}
For the WikiText-2 BPE2048 smoke configs (\texttt{configs/wikitext2\_*\_cuda\_4070m\_bpe2048\_smoke.yaml}),
this yields $\approx 36.96$M FLOPs/token for \APELR{} (GRU(256$\rightarrow$768)$\times$2 + $K{=}8$ experts) and $\approx 32.05$M FLOPs/token for the 2-layer Transformer ($d{=}768$, $h{=}8$) under $T{=}96$.
With adaptive batch sizing (cap 8) and gradient accumulation 2, the effective tokens per optimizer step are $8\times 96\times 2=1536$; the WikiText-2 smoke runs train for 80 steps (total $\approx 1.23\times 10^5$ tokens).
Ignoring auxiliary losses and using a standard $3\times$ multiplier for forward+backward, the total training compute is on the order of 13.6 TFLOPs for \APELR{} and 11.8 TFLOPs for the Transformer.
...
\toprule
Model & Params & FLOPs/token (fwd) & Peak mem/sample (fp16 train) & Titans eval time \\
\midrule
\APELR{} & 19.64M & 36.96M & 233.6 MiB & 69.1 min \\
Transformer & 17.39M & 32.05M & 153.7 MiB & 3.0 min \\
\bottomrule
\end{tabular}
\caption{Compute and resource summary for the WikiText-2 BPE2048 smoke checkpoints.
FLOPs/token are rough forward-pass estimates excluding auxiliary losses; peak memory per sample is from the adaptive-batch probe (fp16 forward+backward).
Titans eval time is the wall-clock time to run \Cref{sec:std-evals} for one checkpoint on the reported hardware (excluding dataset download); \APELR{} timing reflects a correct but unoptimized Python-level per-token belief recursion.}
...
For \APELR{} runs, planner diagnostics indicate that the planner state materially affects emission (mask deltas and forced-state divergences are nontrivial), even at modest training horizons.

\paragraph{Run configurations.}
For reproducibility, key runs correspond to:
\begin{itemize}
  \item \texttt{tinystories\_v2\_cuda\_4070m}: char-level CUDA run from \texttt{configs/tinystories\_v2\_cuda\_4070m.yaml} (seq\_len 128, 220 steps, fp16, grad accumulation 2).
  \item \texttt{v2\_4070\_iter4\_bpe512}: BPE512 CUDA run from \texttt{configs/local/v2\_4070\_iter4\_bpe512.yaml} (hidden 512, 300 steps, fp16, grad accumulation 2).
  \item \texttt{tinystories\_v2\_fastiter\_a1}: BPE4096 CUDA run from \texttt{configs/local/v2\_tinystories\_cuda\_fast.yaml} (39M params, 120 steps).
  \item \texttt{tinystories\_v2\_xl\_a2}: BPE8192 CUDA run from \texttt{configs/local/v2\_tinystories\_cuda\_xl.yaml} (88M params; longer-horizon TinyStories run).
  \item \texttt{wikitext2\_v2\_cuda\_4070m\_bpe2048\_smoke}: BPE2048 CUDA smoke from \texttt{configs/wikitext2\_v2\_cuda\_4070m\_bpe2048\_smoke.yaml} (seq\_len 96, 80 steps).
\end{itemize}
All CUDA runs use adaptive batch sizing to fit 8\,GB VRAM.
...
\toprule
Run & Params & Vocab & Val PPL & Mask $\Delta$ & Force $\JS$ \\
\midrule
tinystories\_v2\_cuda\_4070m & 7.18M & 104 & 10.64 & 0.521 & 0.692 \\
v2\_4070\_iter4\_bpe512 & 5.13M & 512 & 57.62 & 0.164 & 0.209 \\
tinystories\_v2\_fastiter\_a1 & 39.07M & 4096 & 397.23 & -0.008 & 0.328 \\
tinystories\_v2\_xl\_a2 & 88.43M & 8192 & 382.29 & 0.008 & 0.039 \\
wikitext2\_v2\_cuda\_4070m\_bpe2048\_smoke & 19.64M & 2048 & 714.64 & 0.096 & 0.213 \\
\bottomrule
\end{tabular}
\caption{Representative validation results from \texttt{runs/*/metrics.json}. Absolute PPL values are not directly comparable across different tokenizations and training horizons; Mask $\Delta$ and Force $\JS$ are used as coupling diagnostics. Mask $\Delta$ is the NLL increase when replacing planner gating with a uniform gate (planner removed). Force $\JS$ is the JS divergence between forced planner states (intervention diagnostic).}
...
tinystories\_v2\_fastiter\_a1 & 0.079 & 2.190 & 0.379 & -0.008 & 0.328 & 0.966 & -- \\
tinystories\_v2\_xl\_a2 & 0.257 & 2.192 & 0.047 & 0.008 & 0.039 & 0.897 & -- \\
wikitext2\_v2\_cuda\_4070m\_bpe2048\_smoke & 0.403 & 1.329 & 0.394 & 0.096 & 0.213 & 0.806 & 0.101 \\
\bottomrule
\end{tabular}
\caption{\APELR{} planner diagnostics (selected columns) from \texttt{runs/*/metrics.json}. \texttt{usageKL}: KL to uniform usage; \texttt{bndEnt}: boundary entropy; \texttt{planJS}: mean pairwise expert JS; \texttt{mask$\Delta$}: planner mask delta; \texttt{forceJS}: forced-state divergence; \texttt{util}: expert utilization (normalized entropy of usage); \texttt{fb$\Delta$}: NLL improvement from lookahead feedback (when recorded).}
\label{tab:v2-metrics}
\end{table}

\subsection{Standardized zero-shot benchmarks (Titans suite)}
\label{sec:std-evals}
To compare \APELR{} against a decoder-only Transformer baseline using standardized evaluation datasets, we ran a small suite aligned with the benchmarks reported in \emph{Titans: Learning to Memorize at Test Time} \citep{behrouz2025titans}.
We evaluate perplexity on WikiText \citep{merity2016wikitext} and LAMBADA \citep{paperno2016lambada}, and zero-shot multiple-choice accuracy on PIQA \citep{bisk2019piqa}, HellaSwag \citep{zellers2019hellaswag}, ARC-Easy/Challenge \citep{clark2018arc}, WinoGrande (XL) \citep{sakaguchi2020winogrande}, SocialIQA \citep{sap2019socialiqa}, and BoolQ \citep{clark2019boolq}; we additionally include OpenBookQA \citep{mihaylov2018openbookqa}.
Under the \texttt{Salesforce/wikitext} loader, the raw v1 \emph{test} splits for WikiText-2 and WikiText-103 are identical; we report both for compatibility with Titans-style tables.
Both models were trained on WikiText-2 with the same byte-level BPE tokenizer (vocab 2048) and evaluated in a zero-shot manner by scoring continuations with teacher-forced log-likelihood.
For multiple-choice tasks, we report both raw accuracy (\texttt{acc}) and length-normalized accuracy (\texttt{acc\_norm}), where choices are scored by (i) the sum of token log-probabilities in the continuation and (ii) the mean per-token log-probability, respectively.
To keep evaluation comparable given the Transformer's fixed \texttt{max\_seq\_len}, we truncated inputs to a maximum length of 96 tokens (\texttt{--max-input-len 96}).
All raw metrics (including example counts and per-task runtimes) are recorded in \texttt{runs/eval\_titans\_full\_wikitext2\_apel\_vs\_transformer.json}.
...
\toprule
Benchmark & Metric & \APELR{} & Transformer \\
\midrule
WikiText-2 (test) \citep{merity2016wikitext} & PPL $\downarrow$ & 715.219 & 511.030 \\
WikiText-103 (test) \citep{merity2016wikitext} & PPL $\downarrow$ & 715.219 & 511.030 \\
LAMBADA (OpenAI) \citep{paperno2016lambada} & PPL $\downarrow$ & 1036.931 & 841.255 \\
LAMBADA (OpenAI) \citep{paperno2016lambada} & Acc$_{\text{1tok}}\uparrow$ & 0.000 & 0.000 \\
\bottomrule
\end{tabular}
\caption{Perplexity-style evaluation on Titans-aligned benchmarks for two WikiText-2-trained checkpoints (seq\_len 96). ``Acc$_{\text{1tok}}$'' for LAMBADA is computed on examples where the target final word is a single token under the learned BPE tokenizer.}
\label{tab:titans-ppl}
\end{table}
...
\toprule
Task & $N$ & \multicolumn{2}{c}{\APELR{}} & \multicolumn{2}{c}{Transformer} \\
 &  & Acc$\uparrow$ & Acc$_{\text{norm}}\uparrow$ & Acc$\uparrow$ & Acc$_{\text{norm}}\uparrow$ \\
\midrule
PIQA \citep{bisk2019piqa} & 1838 & 0.516 & 0.497 & 0.516 & 0.492 \\
HellaSwag \citep{zellers2019hellaswag} & 10042 & 0.256 & 0.250 & 0.255 & 0.244 \\
ARC-Easy \citep{clark2018arc} & 570 & 0.275 & 0.272 & 0.275 & 0.274 \\
ARC-Challenge \citep{clark2018arc} & 299 & 0.187 & 0.197 & 0.181 & 0.221 \\
WinoGrande-XL \citep{sakaguchi2020winogrande} & 1267 & 0.497 & 0.503 & 0.502 & 0.509 \\
SocialIQA \citep{sap2019socialiqa} & 1954 & 0.334 & 0.309 & 0.332 & 0.325 \\
BoolQ \citep{clark2019boolq} & 3270 & 0.378 & 0.622 & 0.378 & 0.622 \\
OpenBookQA \citep{mihaylov2018openbookqa} & 500 & 0.142 & 0.230 & 0.142 & 0.238 \\
\bottomrule
\end{tabular}
\caption{Zero-shot multiple-choice evaluation (teacher-forced scoring) on Titans-aligned benchmarks \citep{behrouz2025titans}. Acc$_{\text{norm}}$ denotes length-normalized scoring (mean log-prob per continuation token).}
\label{tab:titans-mc}
\end{table}

\paragraph{Interpretation.}
These checkpoints are intentionally small and under-trained (80 optimizer steps on $\approx 1.23\times 10^5$ tokens; \Cref{sec:compute}), so absolute benchmark performance is near chance on many tasks.
We primarily use this suite as a standardized sanity check and a like-for-like comparison between \APELR{} and a Transformer under identical data, tokenizer, and compute budgets.
...
\Cref{tab:ablation} reports controlled ablations on held-out subsets (10 validation batches, batch size 4) for two \APELR{} checkpoints.
...
\label{sec:scaling}
The current experiments are primarily intended to validate the \emph{mechanisms} required for ``think while speak'' (online filtering, expert gating, and forecast coupling) at small/medium scale on a single laptop GPU.
To match the presentation standards of modern architecture papers, the next phase of this project is a multi-scale training sweep up to $\sim$1B parameters with consistent tokenization, token budgets, and standardized evaluation.

\paragraph{Engineering prerequisites.}
Before spending large compute, three implementation issues must be addressed:
(i) remove Python-level per-token overhead in the teacher-forced belief recursion used in training/eval,
(ii) batch and accelerate standardized evaluation (multiple-choice scoring), and
(iii) add distributed training (e.g., FSDP/DDP) and sharded checkpointing for $\gtrsim$300M parameters.

\paragraph{Scaling design choice: output-head cost.}
\APELR{} uses an expert-only emission head with $K$ state-conditional token distributions, which scales output-head compute roughly linearly in $K$.
For large-scale runs we therefore plan to keep $K$ small (e.g., $K\in\{2,4\}$) and/or introduce a factorized expert head that preserves planner dependence without multiplying the full vocab projection.
...
\toprule
Params & Vocab & $K$ & Seq len & Tokens & Notes \\
\midrule
25M & 16k & 4 & 1024 & 1B & single node pilot \\
60M & 16k & 4 & 2048 & 3B & ablations + throughput \\
120M & 32k & 4 & 2048 & 6B & long-context eval \\
300M & 32k & 4 & 4096 & 10B & multi-GPU recommended \\
1B & 32k & 4 & 4096--8192 & 30B & distributed (FSDP) \\
\bottomrule
\end{tabular}
\caption{Proposed scaling sweep (illustrative). All runs should report tokens trained, throughput (tokens/sec), peak memory, and planner diagnostics in addition to loss/perplexity.}
\label{tab:scaling}
\end{table}

\section{Related Work}
\label{sec:related}
\APEL{} sits at the intersection of hierarchical latent sequence models, state-space filtering, and modern autoregressive LMs.
Our emphasis is not a new attention mechanism per se, but a \emph{causal factorization and execution schedule} that makes concurrent ``planner'' computation well-defined.

\paragraph{Transformers and large-scale language modeling.}
The dominant baseline for next-token prediction is the decoder-only Transformer \citep{vaswani2017attention} and its many incremental variants (sometimes informally referred to as ``Transformer++'').
\APEL{} is compatible with Transformer executors, but the key novelty here is explicit separation of a slow plan state from fast token emission.

\paragraph{Linear-time sequence models and state space methods.}
Recent work has explored replacing quadratic attention with linear-time mechanisms, including retention-style recurrences \citep{sun2023retentivenetworksuccessortransformer}, selective state space models \citep{gu2024mambalineartimesequencemodeling}, and gated linear attention \citep{yang2024gatedlinearattentiontransformers}.
Delta-rule updates have been proposed as an efficient way to parallelize certain linear-attention dynamics \citep{yang2025parallelizinglineartransformersdelta}, and gated delta networks combine delta-style updates with modern SSMs \citep{yang2025gateddeltanetworksimproving}.
These lines of work primarily target \emph{sequence modeling efficiency}; \APEL{} instead targets \emph{concurrency of planning and speaking} via a chunk-latent belief and lookahead priors, and can in principle be paired with SSM- or linear-attention executors.

\paragraph{Test-time memory and adaptation.}
Architectures that allocate extra compute at inference time for memory or adaptation (rather than only at training time) are closely related in spirit.
Titans \citep{behrouz2025titans} studies test-time memory mechanisms for LMs, and recent work explores end-to-end test-time training specifically for long-context settings \citep{tandon2025endtoendtesttimetraininglong}.
\APEL{} differs in that the planner computation is designed to run \emph{concurrently} with token emission without violating causality, and its non-interference claims are explicit under the model factorization.

\paragraph{Filtering with hierarchical latents.}
Chunk-level discrete latents with Markov transitions are closely related to HMM-style filtering \citep{rabiner1989hmm} and their neural variants.
\APEL{} can be viewed as an HMM-like filtering recursion where emissions are themselves autoregressive within each chunk.

\paragraph{Mixture-of-experts and gating.}
Gating multiple expert emission distributions connects to mixture-of-experts models \citep{shazeer2017moe}; \APELR{} uses a small $K$-expert emission head controlled by an online planner belief.

\paragraph{Auxiliary objectives and decoding heuristics.}
The future contrastive objective is a direct application of InfoNCE-style representation learning \citep{oord2018cpc}.
Repetition unlikelihood \citep{welleck2020unlikelihood} and nucleus sampling \citep{holtzman2020degeneration} are standard techniques for stabilizing generation and avoiding degenerative loops in small models.

\section{Discussion}
\subsection{What is ``planning'' here?}
In \APELR{}, the planner is a \emph{latent} discrete state distribution used to gate experts.
...
In the current implementation, the planner influences token probabilities via the output gate but does not directly condition GRU hidden dynamics.
...
\label{sec:pitfalls}
This appendix summarizes practical issues that matter for making \APEL{}-style models behave as intended.
...
\APELR{} adopts a strict rule: the chunk-boundary update uses only the \emph{previous} completed chunk summary (see \Cref{sec:boundary-update}).
...
\APELR{} therefore applies token filtering updates during prompt ingestion when enabled.
...
The loss computation explicitly aligns chunk boundaries to \emph{target tokens} so that the boundary update occurs exactly when the generated stream crosses a chunk boundary (the same moment generation would apply it).
...
\section{Qualitative Planner Interventions}
\label{sec:qualitative}
To qualitatively probe whether planner states represent distinct modes, \APELR{} supports forced-state generation (\texttt{--force-state} in \texttt{apelr.sample}).
In the BPE512 TinyStories run, forcing different states changes surface statistics (e.g., local phrasing), consistent with the nonzero forced-state divergence.
...
\paragraph{Example: forced-state outputs (BPE512 TinyStories).}
